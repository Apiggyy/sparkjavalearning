package com.self.relearning.core;

import org.apache.spark.SparkConf;
import org.apache.spark.api.java.JavaPairRDD;
import org.apache.spark.api.java.JavaRDD;
import org.apache.spark.api.java.JavaSparkContext;
import org.apache.spark.api.java.function.Function;
import org.apache.spark.api.java.function.PairFunction;
import scala.Tuple2;

import java.util.List;

public class Top3 {
    public static void main(String[] args) {
        SparkConf conf = new SparkConf().setAppName("Top3").setMaster("local");
        JavaSparkContext sc = new JavaSparkContext(conf);
        JavaRDD<String> lines = sc.textFile("H:/download/test/test04.txt");
        JavaPairRDD<Integer, String> pairs = lines.mapToPair(new PairFunction<String, Integer,
                String>() {
            private static final long serialVersionUID = 1136952989574162002L;

            @Override
            public Tuple2<Integer, String> call(String line) throws Exception {
                return new Tuple2<>(Integer.valueOf(line), line);
            }
        });
        JavaPairRDD<Integer, String> sortedPairs = pairs.sortByKey(false);
        JavaRDD<String> sortedLines = sortedPairs.map(new Function<Tuple2<Integer, String>, String>() {
            private static final long serialVersionUID = 5105790626835381998L;

            @Override
            public String call(Tuple2<Integer, String> t) throws Exception {
                return t._2;
            }
        });
        List<String> list = sortedLines.take(3);
        for (String s : list) {
            System.out.println(s);
        }
        sc.close();
    }
}
